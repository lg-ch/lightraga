        ...
        async with cls._lock:
            if cls._client is None:
                ...
                cls._client = AsyncElasticsearch(
                    hosts=os.environ.get("ELASTICSEARCH_URI", "http://localhost:9200"),
                    basic_auth=auth,
                )
            return cls._client
...
class ElasticsearchKVStorage(BaseKVStorage):
    ...
    async def is_empty(self) -> bool:
              """
              Check if the index contains any document for the current workspace/namespace.
              Returns:
              bool: True if empty, False otherwise.
              """
              try:
                 # Lightweight search, don't return docs
                 response = await self.es_client.search(
                            index=self.index_name,
                            body={"query": {"match_all": {}}},
                            size=0
                 ...
                 if isinstance(total, dict):  # ES >= 7 returns dict {"value": ..., "relation": ...}
                    total = total.get("value", 0)

                 return total == 0

              except NotFoundError:
                 # Index does not exist → considered empty
                 return True
              except Exception as e:
...
class ElasticsearchDocStatusStorage(DocStatusStorage):
    ...
    async def is_empty(self) -> bool:
        """Return True if index contains no documents"""
        try:
    ...
    async def get_doc_by_file_path(self, file_path: str):
        resp = await self.es_client.search(
            index=self.index_name,
            body={"query": {"term": {"file_path.keyword": file_path}}},
            size=1,
        )
        hits = resp["hits"]["hits"]
        return hits[0]["_source"] if hits else None

    async def get_docs_by_track_id(self, track_id: str):
        resp = await self.es_client.search(
            index=self.index_name,
            body={"query": {"term": {"track_id.keyword": track_id}}},
            size=1000,
        )
    ...
    async def get_docs_paginated(
        ...
        resp = await self.es_client.search(
            ...
            body={
                "query": query,
                "sort": [{f"{sort_field}": {"order": sort_direction}}],
            ...
            docs.append(
                (
                    ...
                    DocProcessingStatus(
                        ...
                        chunks_count=d.get("chunks_count", -1),
                        file_path=d.get("file_path", hit["_id"]),
                        chunks_list=d.get("chunks_list", []),
    ...
    async def get_all_status_counts(self) -> dict[str, int]:
        ...
        counts = {
            ...
            for bucket in resp["aggregations"]["status_counts"]["buckets"]
        }
...
@final
@dataclass
class ElasticsearchVectorDBStorage(BaseVectorStorage):
    ...
    async def get_vectors_by_ids(self, ids: List[str]) -> Dict[str, List[float]]:
        if not ids:
            return {}
        resp = await self.es_client.mget(
            index=self.index_name,
            body={"ids": ids},
